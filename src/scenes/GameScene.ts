...
export class GameScene extends Entity {
    ...
    constructor() {
        ...
        this.dialog.setText("Testing Game Scene - Press arrow keys to move player", false); // No typing effect
        ...
        //Recursively loop trough all Entities in this scene
        this.traverseEntities((entity: Entity) => {
            entity.debugConnectToParent = true;
            console.log("Added debugConnectToParent to entity: "+entity.id);
            setTimeout(()=>{
              let cnew = new Circle("c11"+(Math.floor(Math.random()*999)), entity.position.x, entity.position.y, 10);
              overlay.addChild(cnew)
            }, 1000);
        });

        this.addChild(overlay);

        this.addChild(new FPS("FPS", 200, 20))
